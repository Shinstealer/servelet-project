...
<script type="text/javascript">
	...
	$(document).on("click", ".btn_agree", function(){
		...
		mname = $("#input_name"),
		mphone = $("#input_ph"),
		mmailid = $("#input_email_id"),
		mmailurl = $("#input_email_pw");
		
		var id = $.trim(mid.val())
		var regId =/^[a-z0-9_-]{3,16}$/; /*영문자와 숫자만 가능한 정규식*/
		...
		if(pw1 == ""){
			...
			mpw1.focus();
			mpw1.next().text("8~20자 이내 영문자, 숫자, 특수문자만 입력해주세요").css("display", "block");
			return false;
		...
		if(name == ""){
			mname.focus();
			mname.next().text("공백이 있습니다.").css("display", "block");
			return false;
		}
		...
		/*메일 합치기*/
		var mailid = $.trim(mmailid.val());
		var mailurl =$.trim(mmailurl.val());
		...
		if(mailid == ""){
			mmailid.focus();
			mmailid.next().text("필수정보 입니다.").css("display", "block");
			return false;
		}else if(mailurl == ""){
			mmailurl.focus();
			mmailurl.next().text("필수정보 입니다.").css("display", "block");
			return false;
		}else if(!regMail.test(mail)){
			mmailid.select();
			mmailid.next().text("E-mail형식이 올바르지 않습니다.").css("display", "block");
			return false;
		}
...
<body>
	<div id="body_div">
		<header>
			...
			<div id="header_div">
				<a href="index.bizpoll">
					<img alt="showoff"src="img/Slogo.png" id="join_logo">
				</a>
			</div>
			<h2 id="mm">회원정보 수정</h2>
		</header>
		...
		<div id="insert_pw2" class="insert_div">
			...
			<input type="password" id="input_pw2" name="input_ckpw" class="input_class ckeckpw">
			<span class="error">필수 정보입니다 </span>
		</div>
		...
		<div id="insert_name" class="insert_div">
			...
			<input type="text" id="input_name" name="input_name" value="${sessionScope.loginUser.gname }" class="input_class">
			<span class="error">필수 정보입니다 </span>
		</div>
		...
		<div id="insert_ph" class="insert_div">
			...
			<input type="text" id="input_ph" name="input_ph" value="${sessionScope.loginUser.phone }" class="input_class" maxlength=11>
			<span class="error">필수 정보입니다 </span>
		</div>
		...
		<div>
			...
			<input type="text" id="input_email_id" name="input_email_id" value="${sessionScope.loginUser.email }"  class="email input_class">
			<span class="error">필수 정보입니다 </span>
		</div>
		...
		<div id="email2">
			...
			<input type="text" id="input_email_pw" name="input_email_url" value="${sessionScope.loginUser.email }"  class="email" id="mail">
			<span class="error">필수 정보입니다 </span>
			...
			<div><a href="#" id="modify_btn" class ="btn_agree">수정</a></div>
			<div><a href="pwplayupdate.bizpoll" id="PWch_btn" class ="btn_chgpw">비밀번호 변경</a></div>
			<div><a href="delete_acc_page.jsp" id="PWch_btn" class ="btn_delete_acc">회원탈퇴</a></div>
		</form>
		</section>
